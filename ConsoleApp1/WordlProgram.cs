...
//Stage 13: choie random in words
string[] words = { "aahed", "aalii", "abaca", "abaci", "aback", "abaft", "abamp",
...
if (play != null)
{
    if (!string.IsNullOrEmpty(play) && play.Contains("y", StringComparison.OrdinalIgnoreCase) && play.Length == 1)
    {
        //string wordle = words[index];
        string wordle = "glass";
        Console.WriteLine("Wordle is: " + wordle + '\n' + "-------------");
        //Stage 9 : the player has 6 times guess with count < 6
        
        while (count < 6)
        {
            string? word_input = Console.ReadLine();
            int n = 0;
            int m = 0;
            //Stage 10: how many games were played
            Console.WriteLine("Please enter your guess - attempt " + count++ + ":");
            Console.WriteLine("You guessed: " + word_input + "\n" + "-----------------");
            if (word_input != null)
            {

                if (word_input.Length != wordle.Length)
                {
                    Console.WriteLine("Not enough letters!");
                }
                if (!words.Contains(word_input))
                {
                    Console.WriteLine("Your guess is not correct.");
                }
                else
                {
                    //Stage 4: How many letters they have that in the word and in the correct position
                    if (wordle == word_input)
                    {
                        Console.WriteLine("Your answwer correct.");
                        count = 6;
                        continue;
                    }
                    else
                    {
                        char[] arrChar = wordle.ToCharArray();
                        char[] arrArrayCorrect = word_input.ToCharArray();

                        for (int i = 0; i < arrArrayCorrect.Count(); i++)
                        {
                            char item = arrArrayCorrect[i];
                            if (!arrChar.Contains(item))
                            {
                                // Stage 6, 8 : how many letters do not appear in the word at all
                                arrArrayCorrect[i] = '-';
                            ...
                            else
                            {
                                ...
                                {
                                    arrArrayCorrect[i] = '^';
                                    // Stage 8: Show number of letter in arrChar
                                    n++;
                                }
                            }
                        }
                        string word_filter = new string(arrArrayCorrect);
                        char[] array_filter = word_filter.ToCharArray();
                        //Stage 5, 8: how many letters they have that are in the word but in the wrong position
                        //If arrayChar have letters like array-filter: count it
                        ...
                        //check đúng từ, sai vị trí.
                        for (int i = 0; i < array_filter.Length; i++)
                        {

                            //Stage 14: Test the special case
                            //glass | sassy                                            
                            if (arrChar.Contains(array_filter[i]))
                            {
                                //if array_fiter have letters like array char
                                n--;
                                m--;
                                if (n < 0 || m <= 0) // từ đoán có số kí tự nhỏ hơn hoặc bằng từ gốc = *
                                {
                                    array_filter[i] = '*';

                                }
                                else // từ đoán lớn hơn từ gốc sẽ chuyển ký tự thành -
                                {
                                    array_filter[i] = '-';
                                }
                        ...
                        string ans = new string(array_filter);
